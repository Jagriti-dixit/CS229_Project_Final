...
def getWords(inDirPath, audioFile, outDir):
    ...
    tgPath = os.path.join(inDirPath, "annotation")
    #print(tgPath)
...
def performAudioSplicing():
    ...
    dirPath = os.path.join(pwdPath,"suitcase_corpus")
    wavPath = os.path.join(pwdPath, "suitcase_corpus","wav")
    #Set the output path for all the spliced files and create the directory
    outPath = os.path.join(pwdPath, "suitcase_corpus", "spliced_audio")
    ...
    for file_a in audio_files:
        ...
        print("File processing complete for: ", file_a,outPath)
